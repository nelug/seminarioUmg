...
(function(){
    ...
    AngularPrint.directive('printBtn',['$window', '$rootScope', 'ServiceGenerico', '$http', '$localStorage',
    function($window, $rootScope, ServiceGenerico, $http, $localStorage){
        return {
            restrict: 'A',
            link: function(scope, element){
                element.on('click', function(){
                    $rootScope.imprimir = scope.cotizacion;
                    $http.get('/api/v1/detalle-cotizacion/' + scope.cotizacion.id +'?token='+$localStorage.token).success(function(data) {
